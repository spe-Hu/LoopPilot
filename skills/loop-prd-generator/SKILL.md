...
description: "[V3.1] PRD产品需求文档生成器。支持原型驱动、逆向（已有代码）、正向（从零开始）三种入口生成 PRD.json。当用户说'生成PRD'、'创建需求文档'、'从原型生成PRD'时触发。"
---
...
3. **提问补充**（精简，仅问原型不能覆盖的）:
   ...
   - 需要补充的功能？
4. **⚠️ 强制参考示例文件**:
   - `Docs/PRD.json` **必须严格遵循** `skills/loop-prd-generator/examples/sample-prd.json` 的结构
   - 顶层字段：`project_name`, `version`, `last_updated`, `total_tasks`, `summary`
   - **必须使用 `tasks` 数组**，每个 task 必须包含：
     - `id`（TASK-XXX 格式）
     - `user_story`（作为[角色]，我希望[功能]，以便于[价值]）
     - `acceptance_criteria`（L1-L4 分层断言）
     - `unit_test_command` 和 `e2e_test_command`
     - `journey_scenes`（关联 scene ID 数组）
   - **禁止**自行设计其他字段结构（如 `meta`、`pages`、`features`、`models`、`apis`）
...
- **不要自己做决定**——遇到任何疑问、歧义、不确定的地方，必须停下来询问用户
  - 包括但不限于：功能范围模糊、技术选型不确定、用户意图不清、依赖冲突等
  - 询问格式：简述问题 + 提供选项（如有）+ 等待用户确认
